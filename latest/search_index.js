...

{
    ...
    "category": "section",
    "text": "DiffEqBiologicals reaction network is all about chemical arrow notation, so why should we not be able to render arrows?Use latexify\'s env keyword argument to specify that you want :chemical (or the equivalent :arrow, :arrows or :chem).latexify(rn; env=:chemical)requiremhchem  ce varnothing -mathrmhill2left( y v_x k_x right) x ce varnothing -p_y y ce x -d_x varnothing ce y -d_y varnothing ce x -r_b y ce y -r_u x Usually, we want the latexified output to be rendered directly on our screen. This rendering is usually done by Mathjax. To get the chemical arrow notation to render automatically, I have included a Mathjax command (\\require{mhchem}) in the output string. If you want to use the output in a real LaTeX document, you can pass the keyword argument mathjax=false and this extra command will be omitted.  Currently, DiffEqBiological\'s reaction type does not store information about which reactions were bi-directional, so we are only getting unidirectional arrows. However, I have it on good authority that this will be remedied at some point. If this feature is important to you, let us know and we may increase the priority of this issue.Also, latexify does not currently treat leftarrow and Leftarrow differently. In DiffEqBiological, Leftarrow means that mass action is not assumed. It would be easy to fix this, but I do not know how to represent this in LaTeX. If you do, then please open an issue and enlighten us!"
},
